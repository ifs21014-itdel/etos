    ...
    if (row !== null) {
        ...
        $('#product_test_list_detail_dialog').dialog({
            ...
            onLoad: function () {
                ...
                $('#product_test_corrective_action_plan_image').val(row.corrective_action_plan_image);
                $(this).dialog('center');
            }
        });
...
function print_product_test_list(type,view_type) {
    ...
    if (row !== null) {
        if (view_type === 'view')
            ...
            open_target('POST', base_url + 'product_test_list/generate_pdf', {id: row.id,jenis_laporan:view_type}, '_blank');
    } else {
